...
ИСПРАВЛЕННАЯ ВЕРСИЯ — зашумленный сигнал с гармоникой виден чётко
"""
import numpy as np
...
def run():
    ...
    for w in widths:
        ...
        eval_ = "неполное подавление" if w < 2 else ("оптимально" if w <= 3 else "искажения")
        ...
        plot_time_three(
            ...
            ylim=(-0.8, 1.8)  # ← Увеличенный диапазон для видимости гармоники
        )
    ...
    for c_val in c_vals:
        u = create_noisy_signal(g, t, b=0, c=c_val, d=d_fixed)
        U = fftshift(fft(u))
        ...
        if c_val in [0.1, 0.5, 2.0]:
            plot_time_three(
                ...
                ylim=(-1.5, 2.5) if c_val > 1 else (-0.8, 1.8)  # Масштаб под амплитуду
            )

    plot_mse_line(
        [r[0] for r in c_results], [r[1] for r in c_results],
    ...
    print("\n[ВЫВОДЫ 1.5]")
    print("• Оптимальная ширина: 2-3 Гц")
    print("• Эффективность при d > 15 Гц")
    print("• MSE ~ c² подтверждается постоянством MSE/c² ≈ 0.5")